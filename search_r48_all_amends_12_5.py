...
# Find all docx files in R48 dir
def find_docx_files(root_dir):
    docx_files = []
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.endswith(".docx") and "Regulation No. 48" in file:
...
for docx_path in docx_files:
    ...
    if matches_12_5:
        print(f"\n========================================\nFILE: {basename} (Matches: {len(matches_12_5)})\n========================================\n")
        for m in matches_12_5:
            snippet = text[max(0, m.start()-100):min(len(text), m.end()+250)]
